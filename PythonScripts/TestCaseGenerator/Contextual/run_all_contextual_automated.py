...
from PythonScripts.settings import (
    ...
    URL,
    USE_GPU,
    NUM_LLM_RUNS, ALL_CASES, LLM_JSON_PATHCX_AUTOMATED, VOCAB_PATHCX_AUTOMATED, OUTPUT_PATHCX_AUTOMATED
)

import time


def maybe_generate_with_LLM(url: str, LLM_output_path: str = LLM_JSON_PATHCX_AUTOMATED):
    if not USE_LLM:
        return
    ...
    LLM_output_path = LLM_JSON_PATHCX_AUTOMATED
...
def main():
    ...
    if USE_LLM:
        maybe_generate_with_LLM(URL, LLM_output_path=LLM_JSON_PATHCX_AUTOMATED)
        json_paths.append(LLM_JSON_PATHCX_AUTOMATED)

    if ALL_CASES:
        print("\n[4] Building vocabulary...")
        build_vocab_from_json(json_paths, vocab_path=VOCAB_PATHCX_AUTOMATED)
    else:
        print("\n[4] Building vocabulary...")
        build_vocab_from_sjson(LLM_JSON_PATHCX_AUTOMATED, vocab_path=VOCAB_PATHCX_AUTOMATED)

    print("\n[4] Building vocabulary...")
    build_vocab_from_json(json_paths, vocab_path=VOCAB_PATHCX_AUTOMATED)

    print("[5] Loading vocabulary...")
    vocab, _, _ = load_vocab(VOCAB_PATHCX_AUTOMATED)

    print("[6] Training GAN...")
    config = dict(SEQGAN_CONFIG)
    config["output_path"] = OUTPUT_PATHCX_AUTOMATED or "contextual_generated_cases.json"
    ...
    if ALL_CASES:
        ...
        with open("combined_train_automated_cx.json", "w") as f:
            json.dump(combined_data_cx, f)
        trainer.run_pipeline("combined_train_automated_cx.json")
        print(f"Contextual GAN test cases saved to {config['output_path']}")
    else:
        trainer.run_pipeline(LLM_JSON_PATHCX_AUTOMATED)
        print(f"Contextual GAN test cases saved to {config['output_path']}")
